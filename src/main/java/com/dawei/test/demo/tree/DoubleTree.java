...
public class DoubleTree {
	...
	public static void main(String[] args) {
		...
		new DoubleTree().lostRootRangNo(treeNode);

	}

	/**
	 * 二叉树遍历 先序遍历 --- 先根遍历 中序遍历 --- 中根遍历 后序遍历 --- 后根遍历
	 */
	 ...
	 * 后序遍历 左右根 非递归
	 */
	private List<Integer> lostRootRangNo(TreeNode root) {
		List<Integer> pathList = new ArrayList<>();
		if (root == null) {
			return pathList;
		}
		...
		while (!treeNodeStack.isEmpty()) {
			TreeNode parentNode = treeNodeStack.pop();

			if (parentNode.right != null && lastDoNode != parentNode.right) {
				treeNodeStack.push(parentNode);
				TreeNode treeNode = parentNode.right;
				while (treeNode != null) {
					treeNodeStack.push(treeNode);
					treeNode = treeNode.left;
				}
			} else {
				pathList.add(parentNode.val);
				lastDoNode = parentNode;
			}
		}
		return pathList;
	}
